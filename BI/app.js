...
// 1. BI News Dataset (news_data.js 로부터 100건 이상의 실제 크롤링 데이터가 글로벌 주입됨)
// BI_NEWS_DATA 변수는 외부 news_data.js 스크립트에 의해 전역 공간에 사전 로드되어 활용됩니다.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderNewsFeed() {
        ...
        // 전역 변수 BI_NEWS_DATA가 유효한지 검증
        if (typeof BI_NEWS_DATA === 'undefined' || !Array.isArray(BI_NEWS_DATA)) {
            newsCardsContainer.innerHTML = `
                <div class="card-premium" style="grid-column: 1 / -1; padding: 48px; text-align: center;">
                    <i class="fa-solid fa-triangle-exclamation" style="font-size: 3rem; color: #ef4444; margin-bottom: 16px;"></i>
                    <h3 style="font-family: var(--font-display); font-size: 1.25rem; font-weight: 800;">뉴스 데이터를 로드하지 못했습니다</h3>
                    <p style="color: var(--text-muted); margin-top: 8px;">news_data.js 파일의 마운트 유효성을 검사해 주십시오.</p>
                </div>
            `;
            return;
        }
    ...
    function openNewsDetailModal(newsId) {
        ...
        // 원본 구글 뉴스 링크 가져오기
        const sourceLinkHtml = newsItem.raw_link ? `
            <a href="${newsItem.raw_link}" target="_blank" rel="noopener noreferrer" style="color: var(--primary); text-decoration: none; font-weight: 700; display: inline-flex; align-items: center; gap: 4px; margin-top: 10px;">
                <i class="fa-solid fa-square-arrow-up-right"></i> 실제 구글 뉴스 기사 원문 보기
            </a>
        ` : '';
    ...
    function initAnalyticsCharts() {
        if (typeof BI_NEWS_DATA === 'undefined' || !Array.isArray(BI_NEWS_DATA)) return;

window.ReportGenerator = (function() {
  ...
  let reportScatterChart = null;
  let reportPolymerChart = null;
  let reportStyreneVinylChart = null;
  let reportFillersChart = null;
  let reportZnoSulfurChart = null;
  ...
  function init() {
    ...
    const reportScatterX = document.getElementById('report-scatter-x');
    const reportScatterY = document.getElementById('report-scatter-y');
    if (reportScatterX) {
      reportScatterX.addEventListener('change', updateReportScatterChart);
    }
    if (reportScatterY) {
      reportScatterY.addEventListener('change', updateReportScatterChart);
    }
  ...
  function generateReport() {
    ...
    // 실시간 요약 통계 스탯 및 5종 대조 차트 업데이트
    updateReportStats(selected);
    updateReportPolymerChart(selected);
    updateReportStyreneVinylChart(selected);
    updateReportFillersChart(selected);
    updateReportZnoSulfurChart(selected);
    updateReportScatterChart();
  ...
  function updateReportStats(selected) {
    const totalTiresEl = document.getElementById('report-stat-total-tires');
    const totalMakersEl = document.getElementById('report-stat-total-makers');
    const avgTand60El = document.getElementById('report-stat-avg-tand60');
    const avgTgEl = document.getElementById('report-stat-avg-tg');
    const avgG20El = document.getElementById('report-stat-avg-g2_0');

    if (!selected || selected.length === 0) {
      if (totalTiresEl) totalTiresEl.textContent = '0';
      if (totalMakersEl) totalMakersEl.textContent = '0';
      if (avgTand60El) avgTand60El.textContent = '0.0000';
      if (avgTgEl) avgTgEl.textContent = '0.0 ℃';
      if (avgG20El) avgG20El.textContent = '0.00 E+06';
      return;
    }

    // 1. Total Tires
    if (totalTiresEl) totalTiresEl.textContent = selected.length;

    // 2. Unique Makers
    const makers = new Set();
    selected.forEach(t => {
      const maker = window.appState.getProp(t, ['Maker', 'MakerPatternRaw']);
      if (maker && maker !== 'N/A') {
        makers.add(maker.toString().trim());
      }
    });
    if (totalMakersEl) totalMakersEl.textContent = makers.size;

    // 3. Avg tanδ @ 60℃
    let tand60Sum = 0;
    let tand60Count = 0;
    selected.forEach(t => {
      const val = parseFloat(window.appState.getProp(t, ["tanδ @ 60℃", "tanδ @ 60C", "tan δ @ 60℃", "60", "DMTS @ 60℃", "tanδ @ 60℃ (@ 0.5%)"]));
      if (!isNaN(val)) {
        tand60Sum += val;
        tand60Count++;
      }
    ...
    if (avgTand60El) {
      avgTand60El.textContent = tand60Count > 0 ? (tand60Sum / tand60Count).toFixed(4) : '0.0000';
    }
    ...
    let tgCount = 0;
    selected.forEach(t => {
      const val = parseFloat(window.appState.getProp(t, ["Tg_peak temp. (℃)", "Tg_peak temp. (C)", "Tg", "ARES Tg_peak temp. (℃)", "Dynamic Tg"]));
      if (!isNaN(val)) {
        tgSum += val;
        tgCount++;
      }
    ...
    if (avgTgEl) {
      avgTgEl.textContent = tgCount > 0 ? (tgSum / tgCount).toFixed(1) + ' ℃' : '0.0 ℃';
    }
    ...
    let g20Count = 0;
    selected.forEach(t => {
      const val = parseFloat(window.appState.getProp(t, ["G” @ 0℃ (E+06)", "G” @ 0C", "G” @ 0℃", "G'' @ 0℃", "Wet"]));
      if (!isNaN(val)) {
        g20Sum += val;
        g20Count++;
      }
    ...
    if (avgG20El) {
      avgG20El.textContent = g20Count > 0 ? (g20Sum / g20Count).toFixed(2) + ' E+06' : '0.00 E+06';
    }
  }

  // --- Report Specific 5 charts ---
  function updateReportPolymerChart(selected) {
    const ctx = document.getElementById('chart-report-polymer');
    if (!ctx) return;

    if (reportPolymerChart) {
      reportPolymerChart.destroy();
    }
    ...
    reportPolymerChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: labels,
        datasets: [
  ...
  function updateReportFillersChart(selected) {
    const ctx = document.getElementById('chart-report-fillers');
    if (!ctx) return;

    if (reportFillersChart) {
      reportFillersChart.destroy();
    }
    ...
    reportFillersChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: labels,
        datasets: [
  ...
  function updateReportStyreneVinylChart(selected) {
    const ctx = document.getElementById('chart-report-styrene-vinyl');
    if (!ctx) return;

    if (reportStyreneVinylChart) {
      reportStyreneVinylChart.destroy();
    }
    ...
    reportStyreneVinylChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: labels,
        datasets: [
  ...
  function updateReportZnoSulfurChart(selected) {
    const ctx = document.getElementById('chart-report-zno-sulfur');
    if (!ctx) return;

    if (reportZnoSulfurChart) {
      reportZnoSulfurChart.destroy();
    }
    ...
    reportZnoSulfurChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: labels,
        datasets: [
  ...
  function updateReportScatterChart() {
    const ctx = document.getElementById('chart-report-scatter');
    if (!ctx) return;

    if (reportScatterChart) {
      reportScatterChart.destroy();
    }

    const xField = document.getElementById('report-scatter-x').value;
    const yField = document.getElementById('report-scatter-y').value;
    ...
    selected.forEach(item => {
      ...
      if (xField === 'Silica') {
        xVal = getCarbonBlackAndSilica(item)[1];
      } else if (xField === 'Carbon Black') {
        xVal = getCarbonBlackAndSilica(item)[0];
        ...
        const znoSulStr = window.appState.getProp(item, ['Aceton / ZnO / T.Sulfur (phr)', 'Aceton / ZnO / T.Sulfur (phr) ', 'Aceton / ZnO / T.Sulfur', 'Aceton / ZnO / T.Sulfur ']) || '';
      ...
      if (xVal !== null && yVal !== null && !isNaN(xVal) && !isNaN(yVal)) {
        chartData.push({
          ...
          pattern: window.appState.getProp(item, ['Pattern']) || ''
        });
      }
    });

    const r = calculateCorrelation(chartData);
    updateReportCorrelationBadge(r);

    reportScatterChart = new Chart(ctx, {
      type: 'scatter',
      data: {
        datasets: [{
          label: '컴파운드 샘플',
          ...
          tooltip: {
            callbacks: {
              label: function(context) {
                ...
                return ` [${item.maker}] ${item.pattern} (X: ${item.x.toFixed(2)}, Y: ${item.y.toFixed(2)})`;
              }
            }
          }
        },
  ...
  function updateReportCorrelationBadge(r) {
    const badge = document.getElementById('report-scatter-correlation-badge');
    const valElem = document.getElementById('report-correlation-value');
    ...
    valElem.innerHTML = `<span style="font-size: 1rem; font-weight: 700; color: #0f172a;">${rText}</span> <span style="font-size: 0.75rem; font-weight: 600; color: ${color}; margin-left: 6px;">(${desc})</span>`;
    badge.style.borderColor = color;
    badge.style.color = '#0f172a';
    badge.style.background = `linear-gradient(135deg, #ffffff, ${bg})`;
  }

  // --- Duplicate Helper parsing utilities internally for safety ---
